...
def get_best_path(digraph, start, end, path, max_dist_outdoors, best_dist, best_path):
    ...
    path_nodes, path_total_dist, path_outdoor_dist = path

    # Base case: if start and end are the same, return the path
    if start == end:
        return (path_nodes, path_total_dist)
    ...
    for edge in digraph.get_edges_for_node(start_node):
        ...
        result = get_best_path(
            digraph, next_node_name, end, new_path, max_dist_outdoors, best_dist, best_path
        )

        if result is not None:
            result_path_nodes, result_total_dist = result
            if best_dist is None or result_total_dist < best_dist:
                best_path = result_path_nodes
                best_dist = result_total_dist
...
def directed_dfs(digraph, start, end, max_total_dist, max_dist_outdoors):
    ...
    """
    result = get_best_path(
        digraph, start, end, [[start], 0, 0], max_dist_outdoors, None, None
    )
